...
class AppStore extends Component {
  state = {active: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    const {active, searchInput} = this.state
    const tabLst = appsList.filter(each => each.category === active)
    const filteredlst = tabLst.filter(each =>
      each.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )

    return (
      <div className="mainBg">
        <h1 className="appHead">App Store</h1>
        ...
        <ul className="tabCont">
          {tabsList.map(eachItem => (
            <TabItem
              key={eachItem.tabId}
              obj={eachItem}
              isActive={eachItem.tabId === active}
        ...
        <ul className="appList">
          {filteredlst.map(eachItem => (
            <AppItem key={eachItem.appId} obj={eachItem} />
          ))}
        </ul>
      </div>
    )
